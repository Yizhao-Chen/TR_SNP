...
from tkinter import *
from tkinter import simpledialog # For API key/model dialog (alternative)
from tkinter.scrolledtext import ScrolledText # For user request text area
...
def configuration_dialog(fk, file_names_in_csv=True):
    ...
    def apply_correction():        
        ...
        dialog.destroy() # Close the configuration dialog first

        # --- LLM Interaction Start ---
        if messagebox.askyesno("AI Assistant", 
                               "Analysis Complete. Would you like to use an AI assistant to further analyze the data and generate insights/visualizations?",
                               parent=root): # Parent to root window
            
            # Construct placeholder output_csv_paths
            # This is a placeholder. Actual implementation would need to get the real filenames
            # generated by plot_allometry or plot_allometry_species.
            # For now, we'll create a plausible list based on output_path and the first file in fk.
            generated_csv_paths = []
            if fk and output_path:
                base_name = os.path.splitext(os.path.basename(fk[0]))[0]
                # Common output patterns observed in plot_all_allometry.py
                # This is an assumption for the placeholder
                placeholder_suffixes = [
                    "_delta_bio_mean.csv", 
                    "_plot_data_mean.csv", # Assuming a generic plot data csv
                    "_delta_bio_bias_corr_L.csv", # Example if Lockwood method
                    "_delta_bio_mean_bias_corr_L.csv"
                ]
                for suffix in placeholder_suffixes:
                    generated_csv_paths.append(os.path.join(output_path, base_name + suffix))
            
            if not generated_csv_paths: # Fallback if fk or output_path was empty
                generated_csv_paths = [
                    os.path.join(output_path if output_path else ".", "placeholder_delta_bio_mean.csv"),
                    os.path.join(output_path if output_path else ".", "placeholder_plot_data_mean.csv")
                ]

            llm_interaction_dialogs_start(root, generated_csv_paths)
        # --- LLM Interaction End ---
...
# --- LLM Interaction GUI Functions ---
def llm_interaction_dialogs_start(parent_window, output_csv_paths_list):
    """
    Creates the dialog for API key and model name input.
    """
    api_model_dialog = Toplevel(parent_window)
    api_model_dialog.title("AI Assistant Configuration")
    api_model_dialog.geometry("450x280") # Adjusted size for padding
    api_model_dialog.transient(parent_window)
    
    main_frame = ttk.Frame(api_model_dialog, padding="15 15 15 15") # Increased padding
    main_frame.pack(expand=True, fill=tk.BOTH)

    ttk.Label(main_frame, text="LLM Model Name (optional, leave blank for default):").grid(row=0, column=0, sticky=tk.W, pady=(0,5))
    model_name_entry = ttk.Entry(main_frame, width=50)
    model_name_entry.grid(row=1, column=0, sticky=tk.EW, pady=5)

    ttk.Label(main_frame, text="API Key:").grid(row=2, column=0, sticky=tk.W, pady=(10,5))
    api_key_entry = ttk.Entry(main_frame, show='*', width=50)
    api_key_entry.grid(row=3, column=0, sticky=tk.EW, pady=5)

    def on_submit_api_model():
        model_name = model_name_entry.get()
        api_key = api_key_entry.get()
        
        if not api_key.strip(): # Check if API key is empty or just whitespace
            messagebox.showwarning("API Key Required", 
                                 "Please enter your API Key to proceed.", 
                                 parent=api_model_dialog)
            return

        print(f"LLM Model Name: {model_name if model_name else 'Default'}")
        print(f"API Key Entered: {'Yes' if api_key else 'No'}") # Avoid printing key
        
        api_model_dialog.destroy()
        llm_user_request_dialog(parent_window, output_csv_paths_list, model_name, api_key)

    submit_button = ttk.Button(main_frame, text="Next", command=on_submit_api_model)
    submit_button.grid(row=4, column=0, pady=(15,0)) # Increased padding
    
    main_frame.columnconfigure(0, weight=1)
    
    api_model_dialog.grab_set() # Make modal: must be called after window is fully created
    api_model_dialog.lift()
    api_model_dialog.focus_force()
    parent_window.wait_window(api_model_dialog) # Wait for this dialog to close

def llm_user_request_dialog(parent_window, output_csv_paths_list, model_name, api_key):
    """
    Creates the dialog for the user's analysis request.
    """
    request_dialog = Toplevel(parent_window)
    request_dialog.title("AI Analysis Request")
    request_dialog.geometry("500x350")
    request_dialog.transient(parent_window)

    main_frame = ttk.Frame(request_dialog, padding="15 15 15 15")
    main_frame.pack(expand=True, fill=tk.BOTH)

    ttk.Label(main_frame, text="Enter your analysis request (e.g., 'Explain trends in biomass'):").grid(row=0, column=0, sticky=tk.W, pady=(0,5))
    
    request_text_widget = ScrolledText(main_frame, height=10, width=60, wrap=tk.WORD)
    request_text_widget.grid(row=1, column=0, sticky=tk.NSEW, pady=5)

    def on_analyze():
        user_request = request_text_widget.get("1.0", "end-1c").strip()

        if not user_request:
            messagebox.showwarning("Request Required", 
                                 "Please enter your analysis request.", 
                                 parent=request_dialog)
            return

        if not output_csv_paths_list:
            messagebox.showerror("Data Error", 
                                 "No CSV data paths were provided for analysis. Cannot proceed.", 
                                 parent=request_dialog)
            return
            
        # Ensure parent_window is the root or a valid top-level for cursor change
        # If parent_window is request_dialog itself, it will be destroyed.
        # Best to use the main root window (root) if available, or parent of request_dialog.
        # Assuming parent_window is the actual parent passed to llm_user_request_dialog,
        # which should be `root` from `llm_interaction_dialogs_start`.
        
        effective_parent_for_cursor = parent_window 
        # If parent_window is request_dialog, then its parent is parent_window.master
        # However, llm_user_request_dialog is called with parent_window being the root.

        try:
            effective_parent_for_cursor.config(cursor="watch")
            request_dialog.update_idletasks() # Ensure cursor change is visible

            print(f"User Analysis Request: {user_request}")
            print(f"Output CSV Paths for LLM: {output_csv_paths_list}")
            ...
            llm_result = analyze_data_with_llm( 
                ...
                api_key=api_key,
                csv_file_paths=output_csv_paths_list 
            )
        
        finally:
            effective_parent_for_cursor.config(cursor="")
            # No need to update_idletasks for resetting cursor immediately before dialog changes.

        dialog_parent_for_next_step = parent_window 
        request_dialog.destroy() 

        show_llm_analysis_result(dialog_parent_for_next_step, llm_result, output_csv_paths_list)
        
    analyze_button = ttk.Button(main_frame, text="Analyze", command=on_analyze)
    analyze_button.grid(row=2, column=0, pady=(15,0))

    main_frame.columnconfigure(0, weight=1)
    main_frame.rowconfigure(1, weight=1) # Allow text widget to expand

    request_dialog.grab_set() # Make modal
    request_dialog.lift()
    request_dialog.focus_force()
    parent_window.wait_window(request_dialog) # Wait for this dialog to close

def show_llm_analysis_result(parent_window, llm_result, output_csv_paths_list_for_plot_path_derivation):
    """
    Displays the LLM textual analysis in a new Toplevel window with a Save button.
    Also handles initiating plot generation if suggested.
    """
    result_dialog = Toplevel(parent_window)
    result_dialog.title("LLM Analysis Result")
    result_dialog.geometry("700x500")
    result_dialog.transient(parent_window)

    main_frame = ttk.Frame(result_dialog, padding="10 10 10 10")
    main_frame.pack(expand=True, fill=tk.BOTH)

    text_analysis_to_display = llm_result.get("text_analysis", "No text analysis provided or error occurred.")
    if not text_analysis_to_display: 
        text_analysis_to_display = "LLM returned an empty text analysis."

    analysis_text_widget = ScrolledText(main_frame, height=20, width=80, wrap=tk.WORD)
    analysis_text_widget.insert(tk.END, text_analysis_to_display)
    analysis_text_widget.config(state=tk.DISABLED) # Read-only
    analysis_text_widget.grid(row=0, column=0, columnspan=2, sticky="nsew", pady=(0,10))

    def save_analysis_text_content():
        # Use text_analysis_to_display which is already fetched and checked
        if not text_analysis_to_display or "error occurred" in text_analysis_to_display.lower() or "no text analysis provided" in text_analysis_to_display.lower():
            messagebox.showwarning("Save Not Allowed", "Cannot save empty or error message analysis.", parent=result_dialog)
            return

        file_path = filedialog.asksaveasfilename(
            defaultextension=".txt",
            filetypes=[("Text files", "*.txt"), ("Markdown files", "*.md"), ("All files", "*.*")],
            title="Save LLM Analysis As...",
            parent=result_dialog 
        )
        if file_path:
            try:
                with open(file_path, "w", encoding="utf-8") as f:
                    f.write(text_analysis_to_display) 
                messagebox.showinfo("Save Successful", f"Analysis saved to:\n{file_path}", parent=result_dialog)
            except Exception as e:
                messagebox.showerror("Save Failed", f"Could not save analysis: {e}", parent=result_dialog)

    button_frame = ttk.Frame(main_frame)
    button_frame.grid(row=1, column=0, columnspan=2, pady=5)
    
    save_button = ttk.Button(button_frame, text="Save Analysis", command=save_analysis_text_content)
    save_button.pack(side=tk.LEFT, padx=5)

    close_button = ttk.Button(button_frame, text="Close", command=result_dialog.destroy)
    ...
    # Plotting logic, executed after dialog is drawn but before wait_window if plots are blocking
    plot_suggestion = llm_result.get("plot_suggestion")
    if plot_suggestion and isinstance(plot_suggestion, dict) and "error" not in plot_suggestion:
        print(f"Plot suggestion received by show_llm_analysis_result: {plot_suggestion}")
        
        output_data_directory_for_plotter = None
        if output_csv_paths_list_for_plot_path_derivation:
            first_csv_path = output_csv_paths_list_for_plot_path_derivation[0]
            output_data_directory_for_plotter = os.path.dirname(first_csv_path)
            print(f"Derived output_data_directory_for_plotter: {output_data_directory_for_plotter}")
        
        if output_data_directory_for_plotter:
            plot_success, plot_message = generate_visualization(plot_suggestion, output_data_directory_for_plotter)
            if plot_success:
                print("LLM suggested plot was displayed (and potentially saved via its own dialog).")
            else:
                # Use the specific error message from generate_visualization
                detailed_plot_error = plot_message if plot_message else "Could not generate the LLM suggested plot. Unknown error."
                messagebox.showwarning("Plot Generation Failed",
                                       detailed_plot_error,
                                       parent=result_dialog)
        else:
            messagebox.showwarning("Plot Generation Skipped",
                                   "Could not determine the data directory for plotting. This usually means the input CSV file list was empty or there was an issue accessing their path.",
                                   parent=result_dialog)
    elif plot_suggestion and isinstance(plot_suggestion, dict) and "error" in plot_suggestion: # Check type before key access
         messagebox.showwarning("Plot Suggestion Error",
                               f"The plot suggestion from the LLM was malformed and could not be processed: {plot_suggestion['error']}. Raw data: {plot_suggestion.get('raw','')}",
                               parent=result_dialog)
    # No "else" needed here if there's no plot_suggestion, just means no plot to attempt.
                               
    parent_window.wait_window(result_dialog)

# --- End LLM Interaction GUI Functions ---

from llm_analyzer import analyze_data_with_llm # For LLM analysis
from llm_plotter import generate_visualization # For LLM suggested plots
from llm_plotter import generate_visualization # For LLM suggested plots
